                ...
                fetchUserFromAnyClient(data.discordId).then(user => {
                    if (user) user.send({ embeds: [
                        new EmbedBuilder()
                            ...
                            .setDescription(`Sua key \`${name}\` expira **${tsRelative(data.expiry)}**.\n\nAcesse a loja agora para renovar!`)
                            .setTimestamp()
                    ]}).catch(() => {});
                });
            }
            ...
            fetchUserFromAnyClient(p.discordId).then(user => {
                if (user) user.send({ embeds: [
                    new EmbedBuilder()
                        ...
                        .setDescription(`Seu pedido de **${p.label}** foi cancelado após 15 minutos.`)
                        .setTimestamp()
                ]}).catch(() => {});
            });
...
function rateLimitMiddleware(req, res, next) {
    const openRoutes = ["/health", "/", "/dashboard", "/api/dashboard", "/auth/", "/api/admin/"];
    if (openRoutes.some(r => req.path.startsWith(r))) return next();
    const ip = getRealIP(req); const now = Date.now();
    if (blockedIPs[ip]) {
        if (now < blockedIPs[ip]) return res.status(429).json({ status: "error", message: `IP bloqueado.` });
    ...
    if (rateLimitMap[ip].count > RATE_LIMIT_MAX) {
        ...
        return res.status(429).json({ status: "error", message: "Muitas requisições." });
    }
    next();
}
...
function requireAdminAuth(req, res, next) {
    ...
    if (!safeCompare(pass, ADMIN_PASS)) return res.status(401).json({ error: "Sem permissão" });
    next();
}
...
async function confirmarPagamento(user, hours, channel, confirmedBy = "admin", price = null, label = null, couponUsed = null) {
    ...
    if (existingKeyEntry) {
        ...
        const renewEmbed = new EmbedBuilder()
            ...
            .setDescription(`Sua key foi **renovada**!\n\n**🔑 Key:** \`${existingName}\`\n**Plano adicionado:** ${label}\n**Nova expiração:** ${tsRelative(existingData.expiry)}`)
            .setTimestamp();

        let dmOk = false;
        try { await fetchUserFromAnyClient(user.id).then(u => { if(u) u.send({ embeds: [renewEmbed] }); }); dmOk = true; } catch {}
        if (channel) channel.send({ embeds: [new EmbedBuilder().setColor(COLORS.success).setTitle("🔄 Renovação Confirmada").setDescription(`**Usuário:** <@${user.id}>\n**Key:** \`${existingName}\`\n**+Tempo:** ${label}`).setTimestamp()] }).catch(() => {});
        return;
    }
    ...
    try { await fetchUserFromAnyClient(user.id).then(u => { if(u) u.send({ embeds: [dmEmbed] }); }); } catch {}
    if (channel) channel.send({ embeds: [new EmbedBuilder().setColor(COLORS.success).setTitle("✅ Key Gerada").setDescription(`**Usuário:** <@${user.id}>\n**Key:** \`${keyName}\`\n**Expira:** ${tsRelative(expiresAt)}`).setTimestamp()] }).catch(() => {});
...
app.get("/api/transactions", requireAuth, async (req, res) => {
    const transactions = await Transaction.find({ discordId: req.session.user.discordId })
...
app.post("/api/admin/balance", requireAdminAuth, async (req, res) => {
    ...
    await Transaction.create({
        ...
        description: description || "Depósito admin",
    });
...
app.get("/api/admin/users", requireAdminAuth, async (req, res) => {
    ...
    const result = users.map(u => {
        const keyEntry = Object.entries(keys).find(([, d]) => d.discordId === u.discordId);
        return {
            discordId:  u.discordId,
            discordTag: u.discordTag,
            balance:    u.balance,
            hasKey:     !!keyEntry,
            keyName:    keyEntry?.[0] || null,
...
function buildLogsEmbed() {
    return new EmbedBuilder()
        ...
        .setDescription("Gerencie keys, pagamentos, cupons e planos do sistema.")
        .setTimestamp();
}
...
clientLogs.on(Events.InteractionCreate, async (interaction) => {
    ...
    if (id === "logs_stoponline") {
        ...
        await interaction.editReply({ content: "⏹️ Parado." }); return;
    }
    if (id === "logs_stats") {
        ...
        await interaction.editReply({ embeds: [new EmbedBuilder().setTitle("📊 Estatísticas").setColor(COLORS.primary)
            .addFields(
                ...
                { name: "💰 Receita", value: `\`R$${total}\``, inline: true },
            ).setTimestamp()] }); return;
    }
    if (id === "logs_info") {
        await interaction.deferReply({ flags: 64 });
        ...
        const lines = ks.map(k => {
            ...
            return `• \`${k}\`: \`${formatTime(t)}\` ${d.paused ? "⏸️" : "✅"} ${d.discordId ? `<@${d.discordId}>` : ""}`;
        });
    ...
    if (id === "logs_jobids") {
        ...
        if (!entries.length) { await interaction.editReply({ content: "Nenhum JobID." }); return; }
    ...
    if (id === "logs_pendentes") {
        ...
        const list = pendentes.map(p => {
            ...
            return `• **${p.discordTag}** — ${p.label} (R$${p.finalPrice || p.price}) — ⏳ ${rem > 0 ? formatTimeShort(rem) : "expirando..."}`;
        }).join("\n");
        await interaction.editReply({ embeds: [new EmbedBuilder().setColor(COLORS.warning).setTitle(`⏳ Pedidos Pendentes (${pendentes.length})`).setDescription(list)] }); return;
    }
    if (id === "logs_confirmar_manual") {
        await interaction.showModal(new ModalBuilder().setCustomId("modal_pay_confirm").setTitle("Confirmar Pagamento Manual").addComponents(
            ...
            new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId("horas").setLabel("Horas:").setStyle(TextInputStyle.Short).setRequired(true)),
        )); return;
    }
    ...
    if (id === "logs_vendas") {
        ...
        await interaction.editReply({ embeds: [new EmbedBuilder().setColor(COLORS.success).setTitle("💰 Vendas")
            .addFields(
                { name: "📦 Total", value: `\`${sales.length}\``, inline: true },
                { name: "💰 Receita", value: `\`R$${totalR}\``, inline: true },
                { name: "📅 Hoje", value: `\`${hoje.length}\``, inline: true },
                { name: "💵 Hoje R$", value: `\`R$${hojeR}\``, inline: true },
            ).setTimestamp()] }); return;
    }
    if (id === "logs_historico") {
        await interaction.deferReply({ flags: 64 });
        const sales = await SaleHistory.find().sort({ confirmedAt: -1 }).limit(20);
        if (!sales.length) { await interaction.editReply({ content: "Nenhuma venda." }); return; }
        const lines = sales.map(s => `• <@${s.discordId}> — **${s.label}** R$${s.price} — \`${s.keyName}\` — ${tsRelative(s.confirmedAt)}`).join("\n");
        await interaction.editReply({ embeds: [new EmbedBuilder().setColor(COLORS.success).setTitle("📜 Histórico").setDescription(lines.substring(0, 4000)).setTimestamp()] }); return;
    }
    if (id === "logs_coupon_create") {
        await interaction.showModal(new ModalBuilder().setCustomId("modal_coupon_create").setTitle("🎟️ Criar Cupom").addComponents(
            ...
            new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId("discount").setLabel("Desconto:").setStyle(TextInputStyle.Short).setRequired(true)),
            ...
            new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId("maxuses").setLabel("Máximo de usos:").setStyle(TextInputStyle.Short).setRequired(true)),
            new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId("key_pass").setLabel("Senha:").setStyle(TextInputStyle.Short).setRequired(true)),
        )); return;
    }
    if (id === "logs_coupon_list") {
        ...
        const lines = coupons.map(c => `• \`${c.code}\` — **${c.discount}${c.type === "percent" ? "%" : " R$"}** — ${c.usedCount}/${c.maxUses}`).join("\n");
        await interaction.editReply({ embeds: [new EmbedBuilder().setColor(COLORS.gold).setTitle("🎟️ Cupons").setDescription(lines).setTimestamp()] }); return;
    }
    if (id === "logs_plan_edit") {
        await interaction.showModal(new ModalBuilder().setCustomId("modal_plan_edit").setTitle("📦 Editar Plano").addComponents(
            new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId("value").setLabel("ID do plano (1h,2h,4h...):").setStyle(TextInputStyle.Short).setRequired(true)),
            new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId("price").setLabel("Novo preço:").setStyle(TextInputStyle.Short).setRequired(true)),
            new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId("active").setLabel("Ativo? (sim/nao):").setStyle(TextInputStyle.Short).setRequired(true)),
            new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId("key_pass").setLabel("Senha:").setStyle(TextInputStyle.Short).setRequired(true)),
        )); return;
    }
    if (id.startsWith("pay_confirm_")) {
        ...
        await interaction.editReply({ content: `✅ Confirmado!` }); return;
    }
    if (id.startsWith("pay_cancel_")) {
        ...
        await PendingPayment.deleteOne({ discordId: targetId });
        await interaction.editReply({ content: `🗑️ Cancelado.` }); return;
    }
...
async function handleLogsModal(interaction) {
    ...
    if (id === "modal_pay_confirm") {
        ...
        await interaction.editReply({ content: `✅ Key gerada para **${target.tag}**!` }); return;
    }
    if (id === "modal_cancel_pedido") {
        const userId = getField("user_id").replace(/\D/g, ""), motivo = getField("motivo").trim() || "Sem motivo";
        const pending = await PendingPayment.findOne({ discordId: userId });
        if (!pending) { await interaction.editReply({ content: "❌ Nenhum pedido pendente." }); return; }
        await PendingPayment.deleteOne({ discordId: userId });
        await interaction.editReply({ content: `🗑️ Cancelado. Motivo: *${motivo}*` }); return;
    }
    ...
    if (id === "modal_plan_edit") {
        ...
        if (!plan) { await interaction.editReply({ content: `❌ Plano \`${value}\` não encontrado.` }); return; }
        if (!isNaN(price)) plan.price = price;
        plan.active = active;
        await PlanModel.findOneAndUpdate({ value }, { price: plan.price, active }, { upsert: true });
        await interaction.editReply({ content: `✅ Plano \`${value}\` atualizado!` }); return;
    }
    ...
    if (id === "modal_transfer") {
        ...
        await interaction.editReply({ content: `✅ Transferida: \`${t}\` → \`${newName}\`!` }); return;
    }
    if (id === "modal_sethwid") {
        if (wrongPass(getField("key_pass"))) { await interaction.editReply({ content: "❌ Senha incorreta!" }); return; }
        ...
        await interaction.editReply({ content: `✅ HWID definido!` }); return;
    }
    if (id === "modal_lookup") {
...
function buildModal_lifetime()  { return new ModalBuilder().setCustomId("modal_lifetime").setTitle("♾️ Criar Lifetime").addComponents(mkInput("key_name","Nome da key"),mkInput("key_pass","Senha de admin")); }
function buildModal_revoke()    { return new ModalBuilder().setCustomId("modal_revoke").setTitle("🗑️ Revogar Key").addComponents(mkInput("key_name","Nome (ou 'all')"),mkInput("key_pass","Senha de admin")); }
function buildModal_pause()     { return new ModalBuilder().setCustomId("modal_pause").setTitle("⏸️ Pausar Key").addComponents(mkInput("key_name","Nome (ou 'all')"),mkInput("key_pass","Senha de admin")); }
function buildModal_reset()     { return new ModalBuilder().setCustomId("modal_reset").setTitle("🔄 Resetar HWID").addComponents(mkInput("key_name","Nome (ou 'all')"),mkInput("key_pass","Senha de admin")); }
function buildModal_addtime()   { return new ModalBuilder().setCustomId("modal_addtime").setTitle("⏱️ Adicionar Tempo").addComponents(mkInput("key_name","Nome (ou 'all')"),mkInput("key_h","Horas","Ex: 12"),mkInput("key_m","Minutos","Ex: 30",false),mkInput("key_pass","Senha de admin")); }
function buildModal_setexpiry() { return new ModalBuilder().setCustomId("modal_setexpiry").setTitle("📅 Redefinir Expiração").addComponents(mkInput("key_name","Nome da key"),mkInput("key_h","Horas"),mkInput("key_m","Minutos","0",false),mkInput("key_pass","Senha de admin")); }
...
function buildPanelEmbed() {
    return new EmbedBuilder().setTitle("🤖 Bob Auto Joiner — Painel").setColor(COLORS.primary)
        .setDescription("Bem-vindo! Clique nos botões abaixo para gerenciar sua key.");
}
...
clientPanel.on("messageCreate", async (message) => {
    ...
    if (message.channel.type === ChannelType.DM) {
        ...
        if (step === "role_key") {
            ...
            return message.reply(`✅ Discord vinculado!`);
        }
...
clientPanel.on(Events.InteractionCreate, async (interaction) => {
    ...
    if (step) {
        ...
        catch { await interaction.editReply({ content: "❌ Habilite mensagens privadas!" }); }
    }
});
...
function buildShopEmbed() {
    ...
    const planList = activePlans.map(p => `${p.emoji} **${p.label}** — R$${p.price},00`).join("\n");
    return new EmbedBuilder().setColor(COLORS.success).setTitle("🛒 Bob Keys — Loja Oficial")
        .setDescription(`Escolha seu plano:\n\n${planList}\n\n> ⏱️ Você tem **15 minutos** para pagar após escolher.\n> 🔄 Se já tiver key ativa, o tempo será **renovado**!`)
        .setTimestamp();
}
...
clientPayment.on("ready", async () => {
    ...
    } catch (e) { console.error("[PAYMENT] Erro:", e.message); }
});
...
clientPayment.on(Events.InteractionCreate, async (interaction) => {
    ...
    if (id === "buy_cupom") {
        const modal = new ModalBuilder().setCustomId("modal_cupom").setTitle("🎟️ Inserir Cupom").addComponents(
            new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId("coupon_code").setLabel("Código:").setStyle(TextInputStyle.Short).setRequired(true))
        );
    ...
    if (id.startsWith("buy_") && id !== "buy_minhakey" && id !== "buy_cupom") {
        ...
        if (existing) {
            ...
            if (rem > 0) { await interaction.editReply({ content: `⚠️ Você já tem um pedido ativo! Expira em **${formatTimeShort(rem)}**.` }); return; }
        }
    ...
    if (id === "buy_minhakey") {
        ...
        await interaction.editReply({ embeds: [new EmbedBuilder().setColor(COLORS.success).setTitle("🔑 Suas Keys").setDescription(list).setTimestamp()] }); return;
    }
});
...
app.get("/api/dashboard", requireDashAuth, async (req, res) => {
    ...
    res.json({
        ...
        keys: active.slice(0, 50).map(([name, d]) => ({ name, expiry: d.expiry === Infinity ? null : d.expiry, discordId: d.discordId, paused: d.paused })),
        online: online.map(p => p.name),
        pendingOrders: pendentes.map(p => ({ discordTag: p.discordTag, label: p.label, price: p.finalPrice || p.price, createdAt: p.createdAt })),
...
app.get("/dashboard", (req, res) => {
    res.send(`<!DOCTYPE html><html><head><title>Dashboard</title></head><body><h1>Bob Joiner Dashboard</h1><p><a href="/api/dashboard">API</a></p></body></html>`);
});
...
app.post("/api/notify", requireClientHeader, (req, res) => {
    ...
    const payload = { id: Date.now().toString(), title: name || "Brainrot", description: description || name || "Novo!", brainrot: name || "Brainrot", name: name || "Brainrot", jobId: xorObfuscate(jobId) || null, value: String(value || "0"), players: "N/A" };
...
app.get("/clients", requireClientHeader, (req, res) => {
    if (req.query.secret !== SCRIPT_SECRET) return res.status(403).json({ status: "error" });
...
app.post("/push-brainrot", requireClientHeader, (req, res) => {
    ...
    if (secret !== SCRIPT_SECRET) return res.status(403).json({ status: "error" });
...
app.post("/link-discord", requireClientHeader, async (req, res) => {
    ...
    if (cleanId.length < 17) return res.status(400).json({ status: "error", message: "Discord ID invalido." });
    const d = keys[r.keyName];
    if (d.discordId && d.discordId !== cleanId) return res.status(409).json({ status: "error", message: "Key ja vinculada." });
    d.discordId = cleanId; await saveKey(r.keyName); res.json({ status: "ok" });
});
app.post("/report-jobid", requireClientHeader, (req, res) => {
    const { key, secret, name, jobId } = req.query;
    if (secret !== SCRIPT_SECRET) return res.status(403).json({ status: "error" });
    const keyName = findKey(key); if (!keyName) return res.status(403).json({ status: "error" });
