...
async function logSecurityAlert(message) {
    ...
    try {
        ...
        if (ch) await ch.send({ embeds: [new EmbedBuilder().setTitle("🚨 Alerta de Segurança").setColor(0xFF3C3C).setDescription(message).setTimestamp()] });
    } catch (_) {}
}
...
function rateLimitMiddleware(req, res, next) {
    ...
    if (!rateLimitMap[ip] || now - rateLimitMap[ip].windowStart > RATE_LIMIT_WINDOW) {
        rateLimitMap[ip] = { count: 1, windowStart: now }; return next();
    }
...
clientNotifier.on("messageCreate", async (message) => {
    ...
    const payload = { id: Date.now().toString(), title: embed.title || "Bob!", description: embed.description || "Novo Alerta!", brainrot: embed.title || "Brainrot", name: embed.title || "Brainrot", jobId: xorObfuscate(jobId), value, players };
...
function buildModal_revoke()    { return new ModalBuilder().setCustomId("modal_revoke").setTitle("🗑️ Revogar Key").addComponents(new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId("key_name").setLabel("Nome da key (ou 'all')").setStyle(TextInputStyle.Short).setRequired(true)),new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId("key_pass").setLabel("Senha de admin").setStyle(TextInputStyle.Short).setRequired(true))); }
function buildModal_pause()     { return new ModalBuilder().setCustomId("modal_pause").setTitle("⏸️ Pausar / Retomar Key").addComponents(new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId("key_name").setLabel("Nome da key (ou 'all')").setStyle(TextInputStyle.Short).setRequired(true)),new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId("key_pass").setLabel("Senha de admin").setStyle(TextInputStyle.Short).setRequired(true))); }
function buildModal_reset()     { return new ModalBuilder().setCustomId("modal_reset").setTitle("🔄 Resetar HWID").addComponents(new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId("key_name").setLabel("Nome da key (ou 'all')").setStyle(TextInputStyle.Short).setRequired(true)),new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId("key_pass").setLabel("Senha de admin").setStyle(TextInputStyle.Short).setRequired(true))); }
function buildModal_addtime()   { return new ModalBuilder().setCustomId("modal_addtime").setTitle("⏱️ Adicionar Tempo").addComponents(new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId("key_name").setLabel("Nome da key (ou 'all')").setStyle(TextInputStyle.Short).setRequired(true)),new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId("key_h").setLabel("Horas a adicionar").setStyle(TextInputStyle.Short).setRequired(true).setPlaceholder("Ex: 12")),new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId("key_m").setLabel("Minutos a adicionar").setStyle(TextInputStyle.Short).setRequired(false).setPlaceholder("Ex: 30")),new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId("key_pass").setLabel("Senha de admin").setStyle(TextInputStyle.Short).setRequired(true))); }
